...
from backend.db_session import get_all_user, get_user_id, check_schedule, insert_object, update_schedule
from backend.db_model import User, Schedule
...
def update_detect_person(face_id, frame):
    ...
    if face_id == 'unknown':
        save_image(UNKNOWN_PATH, frame)
        return
    users = get_user_id(face_id)
    if len(users) == 0:
        return
    records = check_schedule(today, users[0].id)
    if len(records) > 0:
        if current_time not in records[0].modify:
            modify = records[0].modify + ',' + current_time
            log = Schedule(records[0].user_id, today, current_time, current_time, 'Null', modify)
            update_schedule(log)
    else:
        filename = save_image(os.path.join(DATASET_PATH, face_id), frame)
        log = Schedule(users[0].id, today, current_time, current_time, filename, current_time)
        insert_object(log)
        #
